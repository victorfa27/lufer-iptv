...
export default function App() {
  ...
  const [favorites, setFavorites] = useState(() => new Set(JSON.parse(localStorage.getItem("lufer-favorites") || "[]")));
  ...
  const filtered = useMemo(() => {
    ...
    return channels.filter((channel) => {
      ...
      const inSearch = !q || `${channel.name} ${channel.group}`.toLowerCase().includes(q);
  ...
  async function loadUrl(event) {
    ...
    try {
      ...
      setMessage(error.message || "No se pudo cargar la URL. Puede requerir CORS.");
    }
  }
      ...
      <main className="main">
        ...
        <section className="url-box">
          <form onSubmit={loadUrl}>
            <Link2 size={18} />
            <input value={url} onChange={(e) => setUrl(e.target.value)} placeholder="Pega aquí la URL de tu lista M3U..." />
        ...
        <section className="content-grid">
          ...
          <div className="channels-panel">
            <div className="list-header">
              <div>
                <h2>{showFavorites ? "Favoritos" : activeGroup === "all" ? "Todos los canales" : activeGroup}</h2>
            ...
            <ChannelList channels={filtered} favorites={favorites} onFavorite={toggleFavorite} onSelect={setSelected} selected={selected} />
          </div>
        </section>
      </main>
    </div>
